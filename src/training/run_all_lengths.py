"""
src/training/run_all_lengths.py
================================
Trains ALL 4 models (RF, XGBoost, LSTM, Transformer) for EACH sequence
length in SEQUENCE_LENGTHS list, then reports the single best combination
of (model, sequence_length) across all experiments.

Results saved to:
  results/all_lengths_comparison.csv   -- all experiments
  results/best_overall_summary.json    -- winner
  results/<length>d/                   -- per-length model files

Usage (laptop):
    python src/training/run_all_lengths.py

Usage (HPC SLURM):
    sbatch hpc_train.sh        (hpc_train.sh calls this script)
"""
...
# ============================================================
SEQUENCE_LENGTHS = [30, 60, 90]   # days -- add/remove as needed
...
def ensure_preprocessing():
    """Run preprocessing once if processed CSVs don't exist yet."""
    csvs = list(config.PROCESSED_WEATHER_DIR.glob("*_with_spei.csv"))
    if not csvs:
        logger.info("Processed CSVs not found -- running preprocessing pipeline ...")
...
def load_labeled_frames() -> dict:
    """Load per-field processed CSVs into a dict."""
    frames = {}
    for p in config.PROCESSED_WEATHER_DIR.glob("*_with_spei.csv"):
        ...
        frames[name] = df
    if not frames:
        raise RuntimeError("No processed CSVs found. Run preprocessing first.")
    return frames


def build_sequences_for_length(labeled_frames: dict, seq_len: int):
    """Build (X, y, prev_label, meta) for a given window size."""
...
def evaluate(y_true, y_pred, y_proba=None):
    ...
    report   = classification_report(y_true, y_pred,
                   target_names=config.DROUGHT_CLASSES,
    ...
    return {
        ...
        "severe_f1":         round(report["Severe"]["f1-score"],   4),
        "confusion_matrix":  cm.tolist(),
    }


def print_result(model_name, seq_len, m):
    print(f"  [{seq_len}d] {model_name:22s} "
          f"Acc={m['accuracy']:.4f}  "
...
def train_rf(X_tr, y_tr, X_te, y_te, prev_tr, prev_te, cw):
    t0 = time.time()
    Xf_tr = flatten_sequences(X_tr)
    Xf_te = flatten_sequences(X_te)
    if config.USE_LAG_LABEL_FEATURE:
        Xf_tr = flatten_with_lag(Xf_tr, prev_tr)
        Xf_te = flatten_with_lag(Xf_te, prev_te)
...
def train_xgb(X_tr, y_tr, X_va, y_va, X_te, y_te,
    ...
    Xf_tr = flatten_sequences(X_tr)
    Xf_va = flatten_sequences(X_va)
    Xf_te = flatten_sequences(X_te)
    if config.USE_LAG_LABEL_FEATURE:
        Xf_tr = flatten_with_lag(Xf_tr, prev_tr)
        Xf_va = flatten_with_lag(Xf_va, prev_va)
        Xf_te = flatten_with_lag(Xf_te, prev_te)
...
def train_pytorch(model, X_tr, y_tr, X_va, y_va, X_te, y_te,
    ...
    va_loader  = make_loader(X_va, y_va)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_f1 > best_f1:
            ...
            no_impr += 1
            if no_impr >= patience:
                logger.info(f"    Early stop at epoch {epoch} (val_f1={best_f1:.4f})")
                break
...
def run_for_length(labeled_frames: dict, seq_len: int,
    ...
    """Train all models for one sequence length. Returns list of result dicts."""
    ...
    logger.info(f"Sequences: X={X.shape}  y={y.shape}  lag={'yes' if prev_label is not None else 'no'}")
    ...
    X_va, y_va = X[va], y[va]
    X_te, y_te = X[te], y[te]
    pr_tr = prev_label[tr] if prev_label is not None else np.zeros(len(tr), dtype=np.int64)
    pr_va = prev_label[va] if prev_label is not None else np.zeros(len(va), dtype=np.int64)
    pr_te = prev_label[te] if prev_label is not None else np.zeros(len(te), dtype=np.int64)
    ...
    def record(model_name, m, proba=None):
    ...
    json.dump({"model": "Random Forest", "seq_len": seq_len,
               "scaler": scaler_dict, "metrics": m_rf,
               "label_map": config.LABEL_MAP,
               "feature_names": config.MODEL_INPUT_FEATURES},
              open(save_dir / "random_forest_metadata.json", "w"), indent=2)
    ...
    json.dump({"model": "XGBoost", "seq_len": seq_len,
               "scaler": scaler_dict, "metrics": m_xgb,
               "label_map": config.LABEL_MAP,
               "feature_names": config.MODEL_INPUT_FEATURES},
              open(save_dir / "xgboost_metadata.json", "w"), indent=2)
    ...
    json.dump({"model": "LSTM", "seq_len": seq_len,
               "architecture": {"input_size": n_features, "hidden_size": config.LSTM_HIDDEN_SIZE,
                                 "num_layers": config.LSTM_NUM_LAYERS, "num_classes": 3,
                                 "dropout": config.LSTM_DROPOUT, "bidirectional": True},
               "scaler": scaler_dict, "metrics": m_lstm,
               "label_map": config.LABEL_MAP,
               "feature_names": config.MODEL_INPUT_FEATURES,
               "sequence_length": seq_len},
              open(save_dir / "drought_lstm_metadata.json", "w"), indent=2)
    ...
    json.dump({"model": "Transformer", "seq_len": seq_len,
               "architecture": {"input_size": n_features, "d_model": 128,
                                 "nhead": 4, "num_layers": 3,
                                 "dim_feedforward": 256, "num_classes": 3},
               "scaler": scaler_dict, "metrics": m_tf,
               "label_map": config.LABEL_MAP,
               "feature_names": config.MODEL_INPUT_FEATURES,
               "sequence_length": seq_len},
              open(save_dir / "transformer_metadata.json", "w"), indent=2)

    # ── Ensemble ──────────────────────────────────────
    print(f"\n  Computing Ensemble ({seq_len}d) ...")
    m_ens = run_ensemble(y_te, tf_proba, xgb_proba, lstm_proba)
    record("Ensemble (TF+XGB+LSTM)", m_ens)
    json.dump({"model": "Ensemble", "seq_len": seq_len,
               "weights": {"Transformer": 0.50, "XGBoost": 0.30, "LSTM": 0.20},
               "metrics": m_ens},
              open(save_dir / "ensemble_metadata.json", "w"), indent=2)
...
def main():
    print("\n" + "=" * 65)
    print("  DROUGHT DETECTION -- ALL SEQUENCE LENGTHS COMPARISON")
    print(f"  Lengths: {SEQUENCE_LENGTHS} days  |  Device: {DEVICE}")
    print(f"  Lag label: {config.USE_LAG_LABEL_FEATURE}  |  Models: RF, XGBoost, LSTM, Transformer, Ensemble")
    print("=" * 65)

    # Step 1: Make sure preprocessing is done
    ensure_preprocessing()

    # Step 2: Load labeled frames once (shared across all lengths)
    labeled_frames = load_labeled_frames()
    logger.info(f"Loaded {len(labeled_frames)} fields: {sorted(labeled_frames.keys())}")

    # Step 3: Train all models for each sequence length
    all_results = []
    for seq_len in SEQUENCE_LENGTHS:
        save_dir = RESULTS_DIR / f"{seq_len}d"
        try:
            results = run_for_length(labeled_frames, seq_len, save_dir)
    ...
    # Step 5: Find BEST overall combination
    best_row = df.loc[df["macro_f1"].idxmax()]
    best_model    = best_row["model"]
    best_seq_len  = int(best_row["seq_len"])
    best_macro_f1 = best_row["macro_f1"]
    best_severe_f1 = best_row["severe_f1"]

    # Step 6: Print full comparison table
    print("\n" + "=" * 90)
    ...
    for seq_len in SEQUENCE_LENGTHS:
        ...
        print(f"  {seq_len:2d}d  Best: {best['model']:25s}"
              f"  MacroF1={best['macro_f1']:.4f}"
              f"  SevereF1={best['severe_f1']:.4f}"
              f"  Acc={best['accuracy']:.4f}")

    # Step 8: Overall winner
    print("\n" + "=" * 65)
    print(f"  >>> OVERALL BEST: {best_model} with {best_seq_len}-day window")
    print(f"  >>> Macro F1  = {best_macro_f1:.4f}")
    print(f"  >>> Severe F1 = {best_severe_f1:.4f}")
    print("=" * 65)
    ...
    summary = {
        ...
        "best_severe_f1":   float(best_severe_f1),
    ...
    print(f"  results/all_lengths_comparison.csv")
    print(f"  results/best_overall_summary.json")
    for seq_len in SEQUENCE_LENGTHS:
        print(f"  results/{seq_len}d/  (models + metadata)")
